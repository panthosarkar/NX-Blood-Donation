...
const CustomerBalanceComp: FC = () => {
  const { openModal } = useTemplate();
  const router = useRouter();
  const { invoiceInfo, loading } = useInvoiceInfo();
  const { customerFac, invoiceOwner, invoice } = invoiceInfo;
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        {invoiceOwner && !loading ? (
          <UserInfoComp
            photoUrl={invoiceOwner?.photoUrl}
            name={invoiceOwner?.displayName}
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="space-x-0.5">
            <button
              ...
              onClick={() =>
                openModal("remove-fund", { invoiceOwner, invoice })
              }
              disabled={loading}
            >
            ...
            <button
              ...
              onClick={() => openModal("add-fund", { invoiceOwner, invoice })}
            >
              <div className="relative w-full h-full">
                {/* Default Icon */}
                <div className="block group-hover:hidden w-full h-full transition-colors ease-in-out duration-300">
