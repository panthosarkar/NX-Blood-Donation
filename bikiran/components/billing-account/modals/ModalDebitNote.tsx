...
const ModalContent: FC = () => {
  ...
  useEffect(() => {
    ...
    if (selectedUser && selectedUser.id) {
      get(`/admin/billing/account/${selectedUser?.id}/debit-note-account-info`)
        .then(({ data }) => {
          setData(data);
        })
        .catch((err) => {
...
const ModalDebitNote = () => {
  const { closeModal, modalType } = useTemplate();
  return (
    <Dialog open={modalType === "add-debit-note"} onOpenChange={closeModal}>
