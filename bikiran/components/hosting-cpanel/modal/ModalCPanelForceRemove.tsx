...
const ModalContent: FC = () => {
  ...
  const handleSubmit = () => {
    setLoading(true);
    put(`/admin/hosting/cp-manage/${modalData?.id}/remove`, formData)
      .then((res) => {
...
const ModalCPanelForceRemove = () => {
  const { closeModal, modalType, modalData } = useTemplate();
  return (
    <Dialog open={modalType === "cp-force-remove"} onOpenChange={closeModal}>
      <DialogContent aria-describedby={undefined} className="!w-[400px]">
        <DialogHeader>
          <DialogTitle>Remove CPanel</DialogTitle>
...
export default ModalCPanelForceRemove;
