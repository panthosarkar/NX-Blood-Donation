...
const DropdownMainMenu: React.FC<IMainMenu> = ({
  ...
  return (
    <button
      type="button"
      className={cn(
        "group size-full flex gap-3 items-center justify-between p-[5px] px-4",
        {
          "pl-4": !showLogo,
        }
      )}
      onClick={onClick}
    >
      <div className="flex gap-3 items-center overflow-hidden w-[calc(100%_-_18px)] h-full">
        <div className="overflow-hidden">
          <h3 className="text-primary text-base font-normal overflow-hidden whitespace-nowrap text-ellipsis">
            {data?.length ? value : "Select an option"}
          </h3>
        </div>
      </div>
      ...
      <div className="w-[18px]">
        <IconArrow
          className={cn("w-full h-auto transition-all duration-500", {
