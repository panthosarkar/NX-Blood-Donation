...
import { TApiResponse } from "../global-types";
...
export default function useApi() {
  ...
  const request = async <T>(
    ...
    try {
      ...
      // If you have a way to handle authentication info, you can do it here
      if (headers !== undefined) {
        // Make the request
        apiRequest = AxiosAuth.setUrl(
          `https://bloodapi.deshialumni.com/api${formattedUrl}`
        );
        response = await apiRequest?.[method](payload, headers);
      } else {
        // Make the request
        const apiRequest = AxiosAuth.setUrl(
          `https://bloodapi.deshialumni.com/api${formattedUrl}`,
          query
        );
